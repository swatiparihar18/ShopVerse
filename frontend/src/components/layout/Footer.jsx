        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="col-span-2 md:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-3">
              ...
              <span className="font-display font-bold text-lg text-white">Shop<span className="text-primary-400">Verse</span></span>
            </Link>
          ...
          {[
            { title: 'Shop', links: ['Electronics', 'Fashion', 'Sports', 'Home & Living', 'Beauty', 'Books'] },
            { title: 'Account', links: ['Login', 'Register', 'My Orders', 'Wishlist', 'Track Order'] },
            { title: 'Help', links: ['FAQ', 'Returns Policy', 'Shipping Info', 'Contact Us', 'Privacy Policy'] },
          ].map(col => (
            <div key={col.title}>
              <h4 className="text-white font-semibold text-sm mb-3">{col.title}</h4>
              <ul className="space-y-2 text-sm">
                {col.links.map(l => <li key={l}><a href="#" className="hover:text-primary-400 transition-colors">{l}</a></li>)}
              </ul>
            </div>
          ))}
        </div>
